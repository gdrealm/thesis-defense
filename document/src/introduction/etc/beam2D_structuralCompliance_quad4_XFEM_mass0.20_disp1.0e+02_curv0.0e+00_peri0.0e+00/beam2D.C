...
static const Real sCurPen    = 0.0e-01;
static const Real sDispPen   = 1.0e+02;
static const Real sMassRatio = 0.50;
static const UInt sNumOptIts = 1000;

static const bool sSaveIC = 0;

static const bool sOptFDCheck = false;
static const Real sOptFDEpsilon = 1.0e-04;
static const Real sOptAdvEpsilon = 1.0e-06;
                                 ...
                                 Problem   *aProblem)
{
    ...
    crit = aOptimizer->CreateCriteria(GSOL_INTERFACE_CURVATURE,false,true);
    CritEvalFunc = new FunctionScalarProduct(entries,&weights);
    crit->CreateEvaluationFunction(CritEvalFunc,&indices);
...
//			}
		...
		// Change
//		for ( UInt in = 0; in < numNodes; ++in ) {
//			Advs[in] = InclusionRadius - std::pow( std::pow( model->GetNode( in )->GetGlobalCoords()[0], exponent ) + std::pow( model->GetNode( in )->GetGlobalCoords()[1], exponent ), 1.0 / exponent );
//		}
