...
static const Real sCenterX = 1.466667;
...
static const Real sRadiusUpper = 0.95;
...
static const Real sPerPen    = 0.0e-01;
static const Real sCurPen    = 1.0e-02;
static const Real sDispPen   = 1.0e+02;
static const Real sMassRatio = 0.50;
static const UInt sNumOptIts = 500;

static const enum GlbSolVarType GSOL_CURVATURE_TYPE = GSOL_NORMAL_INTERFACE_MEAN_CURVATURE;

static const bool sSaveIC = 0;

static const bool sOptFDCheck = false;
...
/// Element Properties block
   std::vector<UInt> BlkEpIds;
	...
	elemProp->InitializeProperty( EP_INTERFACE_PARAM, 1.0e+02, 1, NULL, 1, NULL );
	...
	elemProp->SetDirective( (UInt) ED_HAMJAC_NORMAL_HEAVISIDE, HAMJAC_NORMAL_HEAVISIDE_NODAL );
...
/// Materials Block
    ...
    BlkMatIds.push_back(30),BlkMatIds.push_back(30),BlkMatIds.push_back(99),BlkMatIds.push_back(30);
	...
	material->InitializeProperty( MP_HAMJAC_EPSILON, 2.0 * 0.66666666667, 1, NULL, 1, NULL );
...
/// Elements Block
	enum ElementType BlockElemTypes[] = {STRUC_LIN_QUAD4,HAMILTON_JACOBI_NORMAL};
	parseModelHelper.CreateElementsByBaseSetId(model,sBSetIds[0],UN_COUPLED_COUPLED_ELEMENT,NON_IC_ELEMENT,sXfemBlockElemType,BlockElemTypes,BlkEpIds,BlkMatIds);
...
//    solverData->mTimeSolverData->mMaxTime         = solverData->mTimeSolverData->mTimeIts*solverData->mTimeSolverData->mDt;
    ...
    solverData->mTimeSolverData->mJacCheckTol = 0.01;
    solverData->mTimeSolverData->mJacCheckEps = 1.0e-06;
    solverData->mTimeSolverData->mJacCheckIgnore = 1.0e-12;
...
/// Linear solver options
    ...
    const bool dumpStiffMatrixForMatlab                = false;
    if (dumpStiffMatrixForMatlab)
...
/// Post Processing Output Variables
	...
	parseInputHelper.CreateNodalOutputVariables( solutionData,      SOL_LEVELSET_NORMALX, ELEM_VAR, 1, sBSetIds );
	parseInputHelper.CreateNodalOutputVariables( solutionData,      SOL_LEVELSET_NORMALY, ELEM_VAR, 1, sBSetIds );
	parseInputHelper.CreateNodalOutputVariables( solutionData,      SOL_LEVELSET_NORMALZ, ELEM_VAR, 1, sBSetIds );
	parseInputHelper.CreateNodalOutputVariables( solutionData, SOL_LEVELSET_NORMALX_XFEM, ELEM_VAR, 1, sBSetIds );
	parseInputHelper.CreateNodalOutputVariables( solutionData, SOL_LEVELSET_NORMALY_XFEM, ELEM_VAR, 1, sBSetIds );
	parseInputHelper.CreateNodalOutputVariables( solutionData, SOL_LEVELSET_NORMALZ_XFEM, ELEM_VAR, 1, sBSetIds );

	parseInputHelper.CreateNodalOutputVariables( solutionData, SOL_INTERFACE_LEVEL_SET_NORMAL_X, ELEM_VAR, 1, sBSetIds );
	parseInputHelper.CreateNodalOutputVariables( solutionData, SOL_INTERFACE_LEVEL_SET_NORMAL_Y, ELEM_VAR, 1, sBSetIds );
	parseInputHelper.CreateNodalOutputVariables( solutionData, SOL_INTERFACE_LEVEL_SET_NORMAL_Z, ELEM_VAR, 1, sBSetIds );

        parseInputHelper.CreateNodalOutputVariables( solutionData, SOL_LEVELSET_NORMAL_HEAVISIDE_NODAL_XFEM, ELEM_VAR, 1, sBSetIds );
        parseInputHelper.CreateNodalOutputVariables( solutionData, SOL_LEVELSET_NORMAL_HEAVISIDE_GAUSS_XFEM, ELEM_VAR, 1, sBSetIds );
...
//        parseInputHelper.CreateNodalOutputVariables   (solutionData, (SolVarType) (SOL_XFEM000+ie), ELEM_VAR, 1, sBSetIds);
    ...
    parseInputHelper.CreateGlobalOutputVariables  (solutionData, GSOL_INTERFACE_CURVATURE, ELEM_VAR, 1, sBSetIds , 0, NULL);
